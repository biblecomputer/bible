...
pub fn ThemeSidebar(
    ...
    Effect::new(move |_| {
        ...
        if let Some(index) = themes_for_effect
            .iter()
            .position(|t| t.id == current_theme_id)
        {
        ...
        match (e.key().as_str(), e.ctrl_key()) {
            ("j", true) => {
                ...
                if let Some(window) = leptos::web_sys::window() {
                    if let Some(document) = window.document() {
                        if let Some(element) =
                            document.get_element_by_id(&format!("theme-{}", next_index))
                        {
                            if let Some(html_element) =
                                element.dyn_ref::<leptos::web_sys::HtmlElement>()
                            {
            ...
            ("k", true) => {
                ...
                if let Some(window) = leptos::web_sys::window() {
                    if let Some(document) = window.document() {
                        if let Some(element) =
                            document.get_element_by_id(&format!("theme-{}", prev_index))
                        {
                            if let Some(html_element) =
                                element.dyn_ref::<leptos::web_sys::HtmlElement>()
                            {
