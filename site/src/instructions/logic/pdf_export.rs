...
fn get_translated_book_name(book_name: &str) -> String {
    ...
    if let Some(current_translation) = get_current_translation() {
        console::log_1(
            &format!(
                "📚 Current translation: {} ({:?})",
                current_translation.name, current_translation.languages
            )
            .into(),
        );
...
/// Core PDF export business logic
pub fn export_bible_to_pdf<F>(
    bible: &Bible,
    ...
    F: Fn(f32, String) + Clone + 'static,
{
    ...
    console::log_1(
        &format!(
            "📖 Processing {} books for PDF export...",
            bible.books.len()
        )
        .into(),
    );
    ...
    for book in &bible.books {
        ...
        for chapter in &book.chapters {
            ...
            current_layer_ref.use_text(&chapter_title, 14.0, centered_x, current_y, &bold_font);
            ...
            for word in words {
                ...
                if test_line.len() > max_chars_per_line && !line_buffer.is_empty() {
                    ...
                    current_layer_ref.use_text(&line_buffer, 11.0, margin_left, current_y, &font);
    ...
    console::log_1(
        &format!(
            "📊 PDF content complete! Processed {} books, {} chapters, {} verses",
...
pub fn trigger_pdf_download(pdf_bytes: Vec<u8>, filename: &str) {
    ...
    console::log_1(
        &format!(
            "✅ Uint8Array created with length: {}",
            uint8_array.length()
        )
        .into(),
    );
    ...
    let anchor = match document
        ...
        .map_err(|e| e)
        .and_then(|elem| {
            elem.dyn_into::<HtmlAnchorElement>()
                .map_err(|_| wasm_bindgen::JsValue::from_str("Failed to cast to HtmlAnchorElement"))
        }) {
        Ok(a) => {
    ...
    console::log_1(
        &format!(
            "✅ Anchor configured - href: {}, download: {}",
            anchor.href(),
            anchor.download()
        )
        .into(),
