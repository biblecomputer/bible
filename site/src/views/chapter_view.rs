...
pub fn ChapterDetail(
                ...
                if let Some(document) = window.document() {
                    ...
                    if let Some(document_element) = document.document_element() {
                        if let Ok(html_element) =
                            document_element.dyn_into::<web_sys::HtmlElement>()
                        {
                            let _ = html_element
                                .style()
                                .set_property("scroll-behavior", "smooth");
                        }
                    }
                }
            }
                    ...
                    if let Some(document) = window.document() {
                        if let Some(first_range) = verse_ranges.first() {
                            ...
                            if let Some(verse_element) = document.get_element_by_id(&verse_id) {
                                ...
                                if let Ok(html_element) =
                                    verse_element.dyn_into::<web_sys::HtmlElement>()
                                {
                                    ...
                                    if let Some(window) = web_sys::window() {
                                        if let Ok(window_height) = window.inner_height() {
                                            if let Some(window_height) = window_height.as_f64() {
                                                ...
                                                let adjusted_scroll =
                                                    current_scroll - (window_height / 3.0);
                            ...
                            if let Some(heading_element) =
                                document.get_element_by_id("chapter-heading")
                            {
                                if let Ok(html_element) =
                                    heading_element.dyn_into::<web_sys::HtmlElement>()
                                {
                                    let _ = html_element.focus();
                                }
                            }
    ...
    let current_chapter_data = Memo::new(move |_| {
        if let Some(bible) = bible_signal.get() {
            ...
            let book_name = chapter_for_data
                .name
                .split_whitespace()
                .take(chapter_for_data.name.split_whitespace().count() - 1)
                .collect::<Vec<_>>()
                .join(" ");
