...
static DUTCH_TRANSLATION: LazyLock<Translation<'static>> =
    LazyLock::new(|| serde_json::from_str(DUTCH_JSON).expect("Failed to parse dutch.json"));

static ENGLISH_TRANSLATION: LazyLock<Translation<'static>> =
    LazyLock::new(|| serde_json::from_str(ENGLISH_JSON).expect("Failed to parse english.json"));
...
impl<'a> Translation<'a> {
    ...
    pub fn get_book(&self, s: &str) -> Option<&'a str> {
        match s {
            ...
            "ruth" => Some(self.ruth),
            "first_samuel" | "1_samuel" | "1samuel" | "i_samuel" | "isamuel" => {
                Some(self.first_samuel)
            }
            "second_samuel" | "2_samuel" | "2samuel" | "ii_samuel" | "iisamuel" => {
                Some(self.second_samuel)
            }
            "first_kings" | "1_kings" | "1kings" | "i_kings" | "ikings" => Some(self.first_kings),
            "second_kings" | "2_kings" | "2kings" | "ii_kings" | "iikings" => {
                Some(self.second_kings)
            }
            "first_chronicles" | "1_chronicles" | "1chronicles" | "i_chronicles"
            | "ichronicles" => Some(self.first_chronicles),
            "second_chronicles" | "2_chronicles" | "2chronicles" | "ii_chronicles"
            | "iichronicles" => Some(self.second_chronicles),
            "ezra" => Some(self.ezra),
            ...
            "first_corinthians" | "1_corinthians" | "1corinthians" | "i_corinthians"
            | "icorinthians" => Some(self.first_corinthians),
            "second_corinthians" | "2_corinthians" | "2corinthians" | "ii_corinthians"
            | "iicorinthians" => Some(self.second_corinthians),
            ...
            | "1thessalonians"
            | "i_thessalonians"
            | "ithessalonians" => Some(self.first_thessalonians),
            "second_thessalonians"
            | "2_thessalonians"
            | "2thessalonians"
            | "ii_thessalonians"
            | "iithessalonians" => Some(self.second_thessalonians),
            "first_timothy" | "1_timothy" | "1timothy" | "i_timothy" | "itimothy" => {
                Some(self.first_timothy)
            }
            "second_timothy" | "2_timothy" | "2timothy" | "ii_timothy" | "iitimothy" => {
                Some(self.second_timothy)
            }
            ...
            "second_peter" | "2_peter" | "2peter" | "ii_peter" | "iipeter" => {
                Some(self.second_peter)
            }
...
mod tests {
    ...
    fn test_chapter_translation() {
        ...
        // Test book name only
        assert_eq!(
            dutch_translation.get("matthew"),
            Some("Matteüs".to_string())
        );
        ...
        assert_eq!(
            dutch_translation.get("matthew 7"),
            Some("Matteüs 7".to_string())
        );
        assert_eq!(
            dutch_translation.get("john 3"),
            Some("Johannes 3".to_string())
        );
        assert_eq!(
            dutch_translation.get("numbers 7"),
            Some("Numeri 7".to_string())
        );
        assert_eq!(
            dutch_translation.get("first_john 2"),
            Some("I Johannes 2".to_string())
        );
    ...
    fn test_english_translation() {
        ...
        // Test book name only
        assert_eq!(
            english_translation.get("matthew"),
        ...
        // Test chapter references
        assert_eq!(
            english_translation.get("matthew 7"),
            Some("Matthew 7".to_string())
        );
        assert_eq!(
            english_translation.get("john 3"),
            Some("John 3".to_string())
        );
    }
    ...
    fn test_command_palette_search_scenario() {
        ...
        assert!(
            translated_lower.contains(query),
            "Translated name '{}' should contain query '{}'",
            translated_lower,
            query
        );
    }
    ...
    fn test_roman_numeral_translations() {
        ...
        assert_eq!(
            dutch_translation.get("I Samuel"),
            Some("I Samuël".to_string())
        );
        assert_eq!(
            dutch_translation.get("II Samuel"),
            Some("II Samuël".to_string())
        );
        assert_eq!(
            dutch_translation.get("I Kings"),
            Some("I Koningen".to_string())
        );
        assert_eq!(
            dutch_translation.get("II Kings"),
            Some("II Koningen".to_string())
        );
        assert_eq!(
            dutch_translation.get("I Chronicles"),
            Some("I Kronieken".to_string())
        );
        assert_eq!(
            dutch_translation.get("II Chronicles"),
            Some("II Kronieken".to_string())
        );

        // Test chapter references with Roman numerals
        assert_eq!(
            dutch_translation.get("II Kings 7"),
            Some("II Koningen 7".to_string())
        );
        assert_eq!(
            dutch_translation.get("I Samuel 3"),
            Some("I Samuël 3".to_string())
        );

        // Test New Testament books
        assert_eq!(
            dutch_translation.get("I Corinthians"),
            Some("I Korintiërs".to_string())
        );
        assert_eq!(
            dutch_translation.get("II Corinthians"),
            Some("II Korintiërs".to_string())
        );
        assert_eq!(
            dutch_translation.get("I Timothy"),
            Some("I Timoteüs".to_string())
        );
        assert_eq!(
            dutch_translation.get("II Timothy"),
            Some("II Timoteüs".to_string())
        );
        assert_eq!(
            dutch_translation.get("I Peter"),
            Some("I Petrus".to_string())
        );
        assert_eq!(
            dutch_translation.get("II Peter"),
            Some("II Petrus".to_string())
        );
        assert_eq!(
            dutch_translation.get("I John"),
            Some("I Johannes".to_string())
        );
        assert_eq!(
            dutch_translation.get("II John"),
            Some("II Johannes".to_string())
        );
        assert_eq!(
            dutch_translation.get("III John"),
            Some("III Johannes".to_string())
        );

        // Test Revelation
        assert_eq!(
            dutch_translation.get("Revelation"),
            Some("Openbaringen".to_string())
        );
        assert_eq!(
            dutch_translation.get("Revelation 1"),
            Some("Openbaringen 1".to_string())
        );
        assert_eq!(
            dutch_translation.get("Revelation of John"),
            Some("Openbaringen".to_string())
        );
        assert_eq!(
            dutch_translation.get("Revelation of John 22"),
            Some("Openbaringen 22".to_string())
        );
    }
}
