        ...
        move |translation: BibleTranslation| {
            ...
            set_downloading_states.update(|states| {
                if let Some(pos) = states
                    .iter()
                    .position(|(name, _)| name == &translation_short_name)
                {
                    states[pos].1 = true;
                } else {
            ...
            spawn_local(async move {
                match download_translation(&translation).await {
                    ...
                    Err(e) => {
                        ...
                        set_download_error.set(Some(format!(
                            "Failed to download {}: {}",
                            translation.name, e
                        )));
                    }
                ...
                match uninstall_translation(&translation_short_name).await {
                    Ok(_) => {
                        ...
                        let current_selected =
                            get_selected_translation().unwrap_or_else(|| "sv".to_string());
                    ...
                    Err(e) => {
                        ...
                        set_uninstall_error.set(Some(format!(
                            "Failed to uninstall {}: {}",
                            translation_short_name, e
                        )));
                    }
                }
            });
